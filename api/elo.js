...
async function recalculateAll() {
  try {
    ...
    const playersResult = await db.query("SELECT id FROM players WHERE role != 'admin'");
    ...
    const singlesResult = await db.query(
      "SELECT id, creator_id, opponent_id, creator_score, opponent_score, points_type, created_at, 'singles' as match_type FROM matches ORDER BY created_at ASC, id ASC"
    );
    const doublesResult = await db.query(
      "SELECT id, p1_id, p2_id, op1_id, op2_id, team_score, opponent_score, points_type, created_at, 'doubles' as match_type FROM team_matches ORDER BY created_at ASC, id ASC"
    );
