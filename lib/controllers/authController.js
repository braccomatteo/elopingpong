...
const register = async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(
      'INSERT INTO players (name, company, bu, password, role) VALUES ($1, $2, $3, $4, \'player\') RETURNING id, name, company, bu, role, approved',
...
const login = async (req, res) => {
  ...
  try {
    ...
    res.json({
      user: { id: user.id, name: user.name, company: user.company, bu: user.bu, role: user.role, approved: user.approved },
      token
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
const getMe = async (req, res) => {
  try {
    const result = await db.query('SELECT id, name, company, bu, role, approved FROM players WHERE id = $1 AND deleted_at IS NULL', [req.user.id]);
    const user = result.rows[0];
    if (!user) {
