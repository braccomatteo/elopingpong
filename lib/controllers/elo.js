...
async function recalculateAll() {
  try {
    ...
    // Replay all matches — also track biggest upset, weekly ELO snapshots, and ELO history
    let biggestUpsetDiff = 0;
    let biggestUpset = null;
    const oneWeekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    const weekSnapshot = {}; // ELO at the boundary of 7 days ago
    let passedWeekBoundary = false;
    const eloHistory = {}; // per-player array of overall ELO after each match
    for (const id of playerIds) eloHistory[id] = [];

    for (const match of allMatches) {
      const matchDate = new Date(match.created_at);

      // Snapshot all players' overall ELO right before the first match within the last 7 days
      if (!passedWeekBoundary && matchDate >= oneWeekAgo) {
        passedWeekBoundary = true;
        for (const id of playerIds) {
          weekSnapshot[id] = Math.max(computeOverall(playerScores[id]), 0);
        }
      }
      ...
      if (match.match_type === 'singles') {
        ...
        if (eloHistory[cId]) eloHistory[cId].push(Math.max(computeOverall(playerScores[cId]), 0));
        if (eloHistory[oId]) eloHistory[oId].push(Math.max(computeOverall(playerScores[oId]), 0));
        ...
        for (const id of [p1, p2, op1, op2]) {
          if (eloHistory[id]) eloHistory[id].push(Math.max(computeOverall(playerScores[id]), 0));
        }
      }
    }

    // Compute weekly top climber (biggest rank position improvement)
    // If no matches in the last 7 days, snapshot is empty — everyone's gain is 0
    if (!passedWeekBoundary) {
      for (const id of playerIds) {
        weekSnapshot[id] = Math.max(computeOverall(playerScores[id]), 0);
      }
    }

    // Only consider active players (at least 1 game)
    const activeIds = playerIds.filter(id => {
      const t = catCounts[id].games_1v1_21 + catCounts[id].games_1v1_11 + catCounts[id].games_2v2_21 + catCounts[id].games_2v2_11;
      return t > 0;
    });

    // Week-start ranking (sorted by snapshot ELO desc)
    const weekStartRanking = [...activeIds].sort((a, b) => (weekSnapshot[b] ?? 1000) - (weekSnapshot[a] ?? 1000));
    const weekStartRank = {};
    weekStartRanking.forEach((id, i) => { weekStartRank[id] = i + 1; });

    // Current ranking (sorted by final ELO desc)
    const finalElos = {};
    for (const id of activeIds) {
      finalElos[id] = Math.max(computeOverall(playerScores[id]), 0);
    }
    const currentRanking = [...activeIds].sort((a, b) => finalElos[b] - finalElos[a]);
    const currentRank = {};
    currentRanking.forEach((id, i) => { currentRank[id] = i + 1; });

    let weeklyTopName = null;
    let weeklyTopGain = 0; // positions climbed
    for (const id of activeIds) {
      const posUp = (weekStartRank[id] ?? currentRank[id]) - currentRank[id];
      if (posUp > weeklyTopGain) {
        weeklyTopGain = posUp;
        weeklyTopName = playerNames[id];
      }
    }

    // Most consistent player (lowest ELO std dev, min 5 matches)
    let consistentName = null;
    let consistentStddev = Infinity;
    for (const id of activeIds) {
      const hist = eloHistory[id];
      if (!hist || hist.length < 5) continue;
      const mean = hist.reduce((a, b) => a + b, 0) / hist.length;
      const variance = hist.reduce((sum, v) => sum + (v - mean) ** 2, 0) / hist.length;
      const stddev = Math.sqrt(variance);
      if (stddev < consistentStddev) {
        consistentStddev = stddev;
        consistentName = playerNames[id];
      }
    }
    ...
    try {
      await db.query(
        `UPDATE global_stats SET biggest_upset_underdog = $1, biggest_upset_favorite = $2, biggest_upset_elo_diff = $3, weekly_top_name = $4, weekly_top_gain = $5, consistent_name = $6, consistent_stddev = $7 WHERE id = 1`,
        [biggestUpset ? biggestUpset.underdog : null, biggestUpset ? biggestUpset.favorite : null, biggestUpset ? biggestUpset.eloDiff : 0, weeklyTopName, Math.round(weeklyTopGain), consistentName, consistentName ? Math.round(consistentStddev) : null]
      );
