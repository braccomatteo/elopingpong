...
async function recalculateAll() {
  try {
    ...
    // Compute weekly top gainer
    // If no matches in the last 7 days, snapshot is empty — everyone's gain is 0
    if (!passedWeekBoundary) {
      // All matches are older than 7 days — snapshot current state (gain will be 0)
      for (const id of playerIds) {
        weekSnapshot[id] = Math.max(computeOverall(playerScores[id]), 0);
      }
    }
    ...
    let weeklyTopGain = 0;
    for (const id of playerIds) {
      const total = catCounts[id].games_1v1_21 + catCounts[id].games_1v1_11 + catCounts[id].games_2v2_21 + catCounts[id].games_2v2_11;
      if (total < 1) continue;
      const finalOverall = Math.max(computeOverall(playerScores[id]), 0);
      const gain = finalOverall - (weekSnapshot[id] ?? 1000);
      if (gain > weeklyTopGain) {
        weeklyTopGain = gain;
