...
async function recalculateAll() {
  try {
    ...
    // Read all non-admin player IDs and names
    const playersResult = await db.query("SELECT id, name FROM players WHERE role != 'admin' AND deleted_at IS NULL");
    const playerIds = playersResult.rows.map(p => p.id);
    const playerNames = {};
    for (const p of playersResult.rows) playerNames[p.id] = p.name;
