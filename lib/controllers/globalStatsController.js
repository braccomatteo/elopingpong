...
exports.getGlobalChartData = async (req, res) => {
  try {
    ...
    const allPlayers = await db.query("SELECT id FROM players WHERE deleted_at IS NULL AND name != 'Admin'");
    const pIds = allPlayers.rows.map(p => p.id);
    const pScores = {};
    const pGames = {};
    for (const id of pIds) {
      pScores[id] = { score_1v1_21: 1000, score_1v1_11: 1000, score_2v2_21: 1000, score_2v2_11: 1000 };
      pGames[id] = 0;
    }
