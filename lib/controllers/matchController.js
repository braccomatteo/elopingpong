...
exports.getPlayerHistory = async (req, res) => {
  try {
    ...
    const filterName = req.query.name?.trim() || '';
    const filterCompany = req.query.company?.trim() || '';
    const filterBu = req.query.bu?.trim() || '';
    ...
    if (filterName) {
      ...
      conditions.push(`((p1_name ILIKE $${n} AND p1_id != $1) OR (p2_name ILIKE $${n} AND p2_id != $1) OR (op1_name ILIKE $${n} AND op1_id != $1) OR (op2_name ILIKE $${n} AND op2_id != $1))`);
    }
    if (filterCompany) {
      params.push(filterCompany);
      const n = params.length;
      conditions.push(`((p1_company ILIKE $${n} AND p1_id != $1) OR (p2_company ILIKE $${n} AND p2_id != $1) OR (op1_company ILIKE $${n} AND op1_id != $1) OR (op2_company ILIKE $${n} AND op2_id != $1))`);
    }
    if (filterBu) {
      params.push(filterBu);
      const n = params.length;
      conditions.push(`((p1_bu ILIKE $${n} AND p1_id != $1) OR (p2_bu ILIKE $${n} AND p2_id != $1) OR (op1_bu ILIKE $${n} AND op1_id != $1) OR (op2_bu ILIKE $${n} AND op2_id != $1))`);
    }
    ...
    const cteSQL = `
      WITH PlayerHistory AS (
        SELECT id, created_at, points_type, 'singles' as match_type,
          creator_id as p1_id, NULL::uuid as p2_id, opponent_id as op1_id, NULL::uuid as op2_id,
          creator_score as t1_score, opponent_score as t2_score
        FROM matches
    ...
    const joinSQL = `
      SELECT ph.*,
        p1.name as p1_name, p1.company as p1_company, p1.bu as p1_bu,
        p2.name as p2_name, p2.company as p2_company, p2.bu as p2_bu,
        op1.name as op1_name, op1.company as op1_company, op1.bu as op1_bu,
        op2.name as op2_name, op2.company as op2_company, op2.bu as op2_bu
    ...
    const countResult = await db.query(
      `${cteSQL} SELECT COUNT(*) as count FROM (${joinSQL}) as fd ${whereSQL}`,
      params
    );
    const total = parseInt(countResult.rows[0].count);
    ...
    const result = await db.query(
      `${cteSQL} SELECT * FROM (${joinSQL}) as fd ${whereSQL} ORDER BY created_at DESC LIMIT $${limitIdx} OFFSET $${offsetIdx}`,
      [...params, limit, offset]
    );

    res.json({ matches: result.rows, total, page, totalPages: Math.ceil(total / limit) });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
