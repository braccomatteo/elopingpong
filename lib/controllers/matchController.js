...
exports.createMatchAdmin = async (req, res) => {
  ...
  try {
    ...
    // Notify both players
    const namesRes = await db.query('SELECT id, name FROM players WHERE id = ANY($1)', [[creator_id, opponent_id]]);
    const nameMap = {};
    namesRes.rows.forEach(r => { nameMap[r.id] = r.name; });
    const notifMsgOpp = `${nameMap[creator_id] || 'Qualcuno'} ha registrato una partita 1v1 contro di te: ${creator_score}-${opponent_score} (${matchPointsType} pts)`;
    const notifMsgCre = `${nameMap[opponent_id] || 'Qualcuno'} ha registrato una partita 1v1 contro di te: ${opponent_score}-${creator_score} (${matchPointsType} pts)`;
