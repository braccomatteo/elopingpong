...
exports.getPlayerStats = async (req, res) => {
  ...
  try {
    ...
    const rankResult = await db.query(
      "SELECT id FROM players WHERE name != 'Admin' AND deleted_at IS NULL AND (games_1v1_21 + games_1v1_11 + games_2v2_21 + games_2v2_11) > 0 ORDER BY score_overall DESC"
    );
    const totalPlayers = rankResult.rows.length;
    const overallRank = rankResult.rows.findIndex(r => r.id === playerId) + 1;

    // Rank within company
    let companyRank = null, companyTotal = null;
    if (player.company) {
      const compRank = await db.query(
        "SELECT id FROM players WHERE name != 'Admin' AND deleted_at IS NULL AND UPPER(company) = UPPER($1) AND (games_1v1_21 + games_1v1_11 + games_2v2_21 + games_2v2_11) > 0 ORDER BY score_overall DESC",
        [player.company]
      );
      companyTotal = compRank.rows.length;
      companyRank = compRank.rows.findIndex(r => r.id === playerId) + 1;
      if (companyRank === 0) companyRank = null;
    }

    // Rank within BU (DATA only)
    let buRank = null, buTotal = null;
    if (player.bu && player.company && player.company.toUpperCase() === 'DATA') {
      const buRankRes = await db.query(
        "SELECT id FROM players WHERE name != 'Admin' AND deleted_at IS NULL AND UPPER(company) = 'DATA' AND bu = $1 AND (games_1v1_21 + games_1v1_11 + games_2v2_21 + games_2v2_11) > 0 ORDER BY score_overall DESC",
        [player.bu]
      );
      buTotal = buRankRes.rows.length;
      buRank = buRankRes.rows.findIndex(r => r.id === playerId) + 1;
      if (buRank === 0) buRank = null;
    }

    // Get all players and all matches for ELO progression replay
    const allPlayersResult = await db.query("SELECT id, name FROM players WHERE name != 'Admin' AND deleted_at IS NULL");
    ...
    for (const match of allMatches) {
      ...
      if (match.match_type === 'singles') {
        ...
        if (cId === playerId || oId === playerId) {
          ...
          if (!h2h[oppId]) h2h[oppId] = { w: 0, l: 0, lastPlayed: null };
          h2h[oppId][won ? 'w' : 'l']++;
          h2h[oppId].lastPlayed = match.created_at;
...
exports.getPendingPlayers = async (req, res) => {
  try {
    const result = await db.query(
      "SELECT id, name, company, bu FROM players WHERE approved = FALSE AND deleted_at IS NULL AND name != 'Admin' ORDER BY name ASC"
    );
  ...
  try {
    const players = await db.query(
      `SELECT p.id, p.name, p.company, p.bu, p.deleted_at, d.name as deleted_by_name
       ...
       WHERE p.deleted_at IS NOT NULL AND p.name != 'Admin'
