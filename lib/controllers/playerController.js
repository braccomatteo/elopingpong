    ...
    for (const match of allMatches) {
      ...
      if (match.match_type === 'singles') {
        ...
        if (cId === playerId || oId === playerId) {
          ...
          // Track extremes
          if (!bestGain || playerDelta > bestGain.delta) bestGain = { delta: playerDelta, opp: oppName, score: scoreStr, cat };
          if (!worstLoss || playerDelta < worstLoss.delta) worstLoss = { delta: playerDelta, opp: oppName, score: scoreStr, cat };
        ...
        if ([p1, p2, op1, op2].includes(playerId)) {
          ...
          if (!bestGain || playerDelta > bestGain.delta) bestGain = { delta: playerDelta, opp: oppNames, score: scoreStr, cat };
          if (!worstLoss || playerDelta < worstLoss.delta) worstLoss = { delta: playerDelta, opp: oppNames, score: scoreStr, cat };
