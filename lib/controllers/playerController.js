...
exports.getAllPlayers = async (req, res) => {
  try {
    const result = await db.query(
      "SELECT id, name, bu, role, score_overall::float, score_1v1_21::float, score_1v1_11::float, score_2v2_21::float, score_2v2_11::float, games_1v1_21, games_1v1_11, games_2v2_21, games_2v2_11 FROM players WHERE role != 'admin' AND deleted_at IS NULL ORDER BY score_overall DESC"
    );
...
exports.getPlayerStats = async (req, res) => {
  ...
  try {
    ...
    const playerResult = await db.query(
      "SELECT id, name, bu, score_overall::float, score_1v1_21::float, score_1v1_11::float, score_2v2_21::float, score_2v2_11::float, games_1v1_21, games_1v1_11, games_2v2_21, games_2v2_11 FROM players WHERE id = $1 AND deleted_at IS NULL",
      [playerId]
    );
    ...
    const rankResult = await db.query(
      "SELECT id FROM players WHERE role != 'admin' AND deleted_at IS NULL ORDER BY score_overall DESC"
    );
    ...
    const allPlayersResult = await db.query("SELECT id FROM players WHERE role != 'admin' AND deleted_at IS NULL");
    ...
    for (const match of allMatches) {
      ...
      if (match.match_type === 'singles') {
        ...
        if (cId === playerId || oId === playerId) {
          ...
          const opponentId = cId === playerId ? oId : cId;
          if (!h2h[opponentId]) h2h[opponentId] = { w: 0, l: 0 };
          h2h[opponentId][won ? 'w' : 'l']++;
      ...
      if (involvedPlayer) {
        ...
        eloHistory.overall.push({ date, elo: Math.round(overall) });
        eloHistory.score_1v1_21.push({ date, elo: Math.round(s.score_1v1_21) });
        eloHistory.score_1v1_11.push({ date, elo: Math.round(s.score_1v1_11) });
        eloHistory.score_2v2_21.push({ date, elo: Math.round(s.score_2v2_21) });
        eloHistory.score_2v2_11.push({ date, elo: Math.round(s.score_2v2_11) });
      }
    }

    // Build h2h with names
    ...
    if (opponentIds.length > 0) {
      ...
      const nameMap = {};
      for (const r of namesResult.rows) nameMap[r.id] = r.name;
      h2hList = opponentIds.map(id => ({
