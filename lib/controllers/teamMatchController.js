...
exports.updateTeamMatch = async (req, res) => {
  ...
  const allIds = [p1_id, p2_id, op1_id, op2_id];
  if (allIds.some(v => !v)) return res.status(400).json({ error: 'Dati mancanti.' });
  if (new Set(allIds).size !== 4) return res.status(400).json({ error: 'Tutti i giocatori devono essere diversi.' });
  if (Number(team_score) === Number(opponent_score)) return res.status(400).json({ error: 'Non sono ammessi pareggi.' });

  try {
    const pt = Number(points_type) === 11 ? 11 : 21;
    const result = await db.query(
      `UPDATE team_matches SET p1_id=$1, p2_id=$2, op1_id=$3, op2_id=$4, team_score=$5, opponent_score=$6, points_type=$7
       WHERE id=$8 AND deleted_at IS NULL RETURNING id`,
      [p1_id, p2_id, op1_id, op2_id, team_score, opponent_score, pt, id]
    );
